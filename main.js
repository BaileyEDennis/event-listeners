...
const handleButtonClick = (e) => {
  ...
  if (buttonID === "Abbey") {
    ...
    bodySelector.style.backgroundColor = "rgb(175, 196, 123)";
  }

  if (buttonID === "All" || buttonID === e.currentTarget.id) {
    selectedPies = pieBuilder(pies);
  } else {
...
const pieBuilder = (monkeyButts) => {
  ...
  for (let i = 0; i < monkeyButts.length; i++) {
    domString += `<div class="card my-2" style="width: 18rem;">
                        ...
                        <button type="button" class="btn btn-info" id="${i}">Delete</button>
                    </div>`;
  }
...
const buttonEvents = () => {
  document
    .querySelector("#intsructor-buttons")
